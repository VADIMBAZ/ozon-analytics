...
if unit_economics.empty:
    ...
    if unit_sel.empty:
        ...
        with tab_dyn:
            ...
            _t.markdown('**Маржа чистая % по месяцам**')
            with _i.popover('ℹ️'):
                st.markdown("""
**Маржа чистая %** = Прибыль чистая / Выручка × 100

**Прибыль чистая** = Прибыль за период (по версии Озон) − Себестоимость × (Доставлено − Возвращено).

- Толстая тёмная линия — агрегат по всем выбранным SKU.
- Цветные линии (если выбраны в multiselect выше) — отдельные артикулы для сравнения с агрегатом.

Скачок: маржа Озона зависит от баллов за скидки, акций, рекламы. Sep 2025 — пик за счёт компенсаций Озон (~93К ₽).
""")
            ...
            _t, _i = st.columns([10, 1])
            _t.markdown('**Прибыль чистая ₽ по месяцам (по артикулам)**')
            with _i.popover('ℹ️'):
                st.markdown("""
**Прибыль чистая ₽** = Прибыль за период (Озон) − Себестоимость × проданных штук.

Каждая группа на оси X = месяц. Внутри группы — отдельные столбики на каждый SKU своего цвета (как в «Остатках на конец месяца и приходы»).

**Как читать:** видно вклад каждого артикула в прибыль помесячно. Если у SKU столбик низкий или отсутствует — значит в этом месяце он либо не продавался, либо ушёл в стокаут.
""")
            ...
            if len(months_ordered) > 0 and len(selected_sorted) > 0:
                ...
                if not pivot_margin.empty:
                    ...
                    _t, _i = st.columns([10, 1])
                    _t.markdown('**Маржа % — SKU × месяц (тепловая карта)**')
                    with _i.popover('ℹ️'):
                        st.markdown("""
**Тепловая карта** маржи: строки — артикулы, столбцы — месяцы, цвет — маржа чистая %.

Шкала: красный → 0%, жёлтый → 30%, зелёный → 50%+.

**Зачем:** мгновенно видно «слабые» сочетания SKU × месяц. Например, **10SK2024 в Feb 2026 = −5.4%** — товар работал в минус. Белые клетки = нет unit-данных (полный стокаут или артикула не было в файле).
""")
            ...
            pivot_pps = (
                unit_month.pivot_table(
                    index='Артикул', columns='month_label',
                    values='Прибыль_на_шт', aggfunc='mean',
                )
                .reindex(
                    index=[a for a in selected_sorted if a in unit_month['Артикул'].values],
                    columns=months_ordered,
                )
                .astype('float64')
            )
            if not pivot_pps.empty:
                ...
                with _i.popover('ℹ️'):
                    st.markdown("""
**Прибыль на штуку** = Прибыль чистая / (Доставлено − Возвращено) для каждой пары (SKU, месяц).
...
""")
        ...
        with tab_struct:
            ...
            _t, _i = st.columns([10, 1])
            _t.markdown('**Расходы и прибыль помесячно, ₽ (стек по статьям)**')
            with _i.popover('ℹ️'):
                st.markdown("""
**Один бар = один месяц.** Высота бара = выручка минус компенсации (объём «оборота» за месяц).

Внутри бара — раскладка по статьям:
- 🔴 **Комиссия Озон** — вознаграждение маркетплейса + эквайринг
- 🟠 **Логистика** — логистика + доставка до ПВЗ + обработка отправления + размещение
- 🟣 **Возвраты** — обработка возврата + обратная логистика
- 🔵 **Реклама** — оплата за клик + оплата за заказ
- ⚪ **Себестоимость** — задаётся в сайдбаре (₽/шт × проданных)
- 🟢 **Прибыль чистая** — то, что осталось после всех расходов

**Как читать:** растёт ли «оборот» (высота столбика), не съедает ли какая-то статья всё больше выручки от месяца к месяцу.
""")
            ...
            if n_m == 0:
                ...
                _t, _i = st.columns([10, 1])
                _t.markdown(f'**Куда уходит выручка по месяцам ({sku_choice})**')
                with _i.popover('ℹ️'):
                    st.markdown("""
**Каждый мини-водопад = один месяц.** Все месяцы расположены на одной шкале Y — удобно сравнивать «выручка → прибыль» помесячно.

Шаги внутри одного водопада:
1. **Выручка** (синий) — то, что покупатели заплатили.
2. **+ Компенсации** (зелёный) — баллы за скидки + программы партнёров (Озон возвращает).
3. **− Комиссия / Логистика / Возвраты / Реклама / Себестоимость** (красные шаги вниз).
4. **Прибыль** (тёмный итог).

**Что искать:** месяцы, где красные шаги непропорционально велики (рост одной из статей), или где «Прибыль» резко проседает.

Селектор сверху позволяет показать водопады по одному артикулу.
""")
